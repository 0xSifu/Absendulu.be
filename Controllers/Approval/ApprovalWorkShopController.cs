...
[ApiController]
public class ApprovalWorkShopController : ControllerBase
{
    private readonly IApprovalWorkShopService _service;
    private readonly TokenValidate _tokenConfig;
    private readonly IApprovalWorkShopLogService _approvalLogs;

    public ApprovalWorkShopController(IApprovalWorkShopLogService approvalWorkShopLogService, IApprovalWorkShopService approvalWorkShopService, TokenValidate tokenValidate)
    {
        _service=approvalWorkShopService;
        _tokenConfig=tokenValidate;
        _approvalLogs=approvalWorkShopLogService;
    }
    ...
    public async Task<ActionResult<List<ApprovalWorkShopLogs>>> GetApprovalDetails([FromQuery] Guid id)
    {
        try
        {
            var responseContext = await Task.Run(() => _approvalLogs.GetApprovalsLogByIdApproval(_tokenConfig.CompanyId,id));
            return Ok(new ResponseMessage<List<ApprovalWorkShopLogs>> { IsError = false, Message = "Success", Data = responseContext });
        }
        catch
        {
            throw;
    ...
    public async Task<ActionResult<List<ApprovalWorkShop>>> GetPagination([FromQuery] PaginationFilter filter)
    {
        try
        {
            ...
            return Ok(new PagedResponse<List<ApprovalWorkShop>>(paging, validFilter.PageNumber, validFilter.PageSize, totalPages, totalRecords, "suceess", true, null));
        }
        catch
        {
            throw;
    ...
    [ServiceFilter(typeof(AuthorizeTokenFilterService))]
    public async Task<ActionResult<List<ApprovalWorkShopLogs>>> GetPaginationApprovalLogs([FromQuery] PaginationFilter filter)
    {
        try
        {
            var validFilter = new PaginationFilter(filter.PageNumber, filter.PageSize, filter.search.ToLower());
            ...
            .Skip((validFilter.PageNumber - 1) * validFilter.PageSize)
            .Take(validFilter.PageSize).ToList();
            return Ok(new PagedResponse<List<ApprovalWorkShopLogs>>(paging, validFilter.PageNumber, validFilter.PageSize, totalPages, totalRecords, "suceess", true, null));
        }
        catch
        {
            throw;
    ...
    public async Task<ActionResult<List<ApprovalWorkShop>>> GetPaginationApprover([FromQuery] PaginationFilter filter)
    {
        try
        {
            var validFilter = new PaginationFilter(filter.PageNumber, filter.PageSize, filter.search.ToLower());
    ...
    [ServiceFilter(typeof(AuthorizeTokenFilterService))]
    public async Task<ActionResult> AddApproval(RequestApprovalWorkShopDTO request)
    {
        try
        {
            var responseContext = await Task.Run(() => _service.AddApproval(request));
            return Created("", new ResponseMessage<ApprovalWorkShop> { IsError = false, Message = "Success", Data = responseContext });
        }
        catch
        {
            throw;
    ...
    public IActionResult Put(Guid Id,[FromBody] ApprovalDTO request)
    {
        try
        {
            ...
            return Ok(new ResponseMessage<ApprovalWorkShop> { IsError = false, Message = "Success", Data = responseContext });
        }
        catch
        {
            throw;
    ...
    public IActionResult ApproveAll([FromQuery] string note)
    {
        try
        {
            ...
            return Ok(new ResponseMessage<List<ApprovalWorkShop>> { IsError = false, Message = "Success", Data = responseContext });
        }
        catch
        {
            throw;
